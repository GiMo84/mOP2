classdef mOP2block_OQG < mOP2block
	%OP2BLOCK_OQG Reads table of single or multipoint constraint forces
	%   OP2BLOCK_OQG reads the table of single or multipoint constraint forces
	...
	%
	%   Block described in MD Nastran 2006 DMAP Programmer's Guide, page 605.
	...
	methods
		function block = mOP2block_OQG(blockInfo, fid, tableFmt)
			block = block@mOP2block(blockInfo, fid, tableFmt);
			block.interpreter = 'OQG';
		end % mOP2block_OQG
		function data = get.data(block)
			...
			if isempty(strmatch('OQG', blockInfo.name)) && isempty(strmatch('OPG', blockInfo.name)) && isempty(strmatch('OQM', blockInfo.name))
				error('mOP2block_OQG: Wrong block type')
			end
					...
					case 1
						switch mOP2block.functionCodes(data(recordID).ACODE, 4)
							...
							case {8} % PostBuckling
								...
								data(recordID).EIGN = fread(fid, 1, 'float32');
								fseek(fid, 4, 0);
							case {10} % Nonlinear statics (e.g. SOL400)
								data(recordID).LFTSFQ = fread(fid, 1, 'int32');
								fseek(fid, 2*4, 0);
							case {6} % Transient
								data(recordID).TIMESTEP = fread(fid, 1, 'float32');
								fseek(fid, 2*4, 0);
							otherwise
								warning('mOP2block_OQG: ACODE %d not implemented', data(recordID).ACODE)
								fseek(fid, 3*4, 0);
						end
					case 2
						data(recordID).LSDVMN = fread(fid, 1, 'int32');
			...
			% 	data(recordID).num_nodes = fread(fid, 1, 'int32') / (data(recordID).NUMWDE*4);
				...
				switch mOP2block.functionCodes(data(recordID).TCODE, 1)
					...
					case 2
						switch mOP2block.functionCodes(data(recordID).ACODE, 4)
							...
							otherwise
								error('mOP2block_OQG: wrong ACODE')
						end
				end
				...
				switch mOP2block.functionCodes(data(recordID).TCODE, 7)
					...
					otherwise
						error('mOP2block_OQG: wrong TCODE')
				end
